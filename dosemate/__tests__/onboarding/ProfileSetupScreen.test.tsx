...
jest.mock('@/components/progress-bar', () => {
  ...
  return ({ step, totalSteps }: any) => (
    <View testID="progress-bar">
      <Text>Progress: {step}/{totalSteps}</Text>
    </View>
  );
});
...
jest.mock('@/components/profile-setup', () => {
  ...
  return {
    ...
    HealthInfo: ({ conditions, selectedConditions, setConditions, allergies, setAllergies }: any) => (
        ...
        {conditions.map((condition: string) => (
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              if (isSelected) {
                setConditions(selectedConditions.filter((c: string) => c !== condition));
              } else {
                setConditions([...selectedConditions, condition]);
              }
            }}
          >
            <Text>{condition} {selectedConditions.includes(condition) ? '✓' : ''}</Text>
          </TouchableOpacity>
        ))}
        <TextInput
    ...
    Lifestyle: ({ sleepSchedules, activityLevels, selectedSleep, setSleep, selectedActivity, setActivity }: any) => (
      <View testID="lifestyle">
        ...
        {sleepSchedules.map((schedule: any) => (
          ...
          >
            <Text>{schedule.label} {selectedSleep === schedule.value ? '✓' : ''}</Text>
          </TouchableOpacity>
        ))}
        {activityLevels.map((level: any) => (
          ...
          >
            <Text>{level.label} {selectedActivity === level.value ? '✓' : ''}</Text>
          </TouchableOpacity>
        ))}
      </View>
    ),
...
describe('ProfileSetupScreen', () => {
  ...
  describe('Step 1 - Personal Info', () => {
    it('enables continue button when age is entered', () => {
      const { getByTestId, UNSAFE_getAllByType } = render(<ProfileSetupScreen />);
    ...
    it('navigates to step 2 when continue is pressed', () => {
      const { getByTestId, getByText, queryByTestId } = render(<ProfileSetupScreen />);
      const ageInput = getByTestId('age-input');
  ...
  describe('State Persistence', () => {
    it('maintains profile data across all steps', () => {
      ...
      expect(screen.getByText('Diabetes ✓')).toBeTruthy();
      expect(screen.getByTestId('allergies-input').props.value).toBe('Penicillin');
