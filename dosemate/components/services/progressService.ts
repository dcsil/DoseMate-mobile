...
function normalizeCreatePayload(data: CreateProgressInput): { metric_name: string; value: number; int_value: number | null } {
...
export async function createProgress(userId: string, data: CreateProgressInput): Promise<ProgressEntry> {
...
export async function listProgress(userId: string, opts?: { metric_name?: string; limit?: number }): Promise<ProgressEntry[]> {
    const params = new URLSearchParams();
    if (opts?.metric_name) params.append('metric_name', opts.metric_name);
    if (opts?.limit) params.append('limit', String(opts.limit));
    const query = params.toString();
    const resp = await fetch(`${BACKEND_BASE_URL}/users/${userId}/progress${query ? `?${query}` : ''}`);
    if (!resp.ok) {
        const text = await resp.text();
...
export async function getLatestProgress(userId: string, metricName: string): Promise<ProgressEntry | null> {
    const entries = await listProgress(userId, { metric_name: metricName, limit: 1 });
